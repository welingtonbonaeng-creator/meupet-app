...
import {
  ArrowLeft, Edit2, Trash2, Plus, Scale, BookOpen,
  Calendar, Syringe, Activity, ChevronRight, AlertTriangle
} from 'lucide-react'

const DIARY_TYPES: DiaryType[] = ['vaccine','appointment','deworming','exam','medication','bath','grooming','surgery','weight','note']
...
export default function PetDetailPage() {
  ...
  const [weightForm, setWeightForm] = useState({ weight_kg: '', notes: '', measured_at: new Date().toISOString().slice(0,10) })

  if (!pet && pets.length > 0) {
    router.push('/pets'); return null
  }
  if (!pet) return (
  ...
  return (
    <div>
      <TopBar
        ...
        leftAction={<button onClick={() => router.back()} className="p-2 rounded-xl hover:bg-slate-100"><ArrowLeft size={20} /></button>}
      />
      ...
      <div className="p-4 lg:p-6 max-w-2xl space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-2 mt-3">
                  <Link href={`/pets/${id}/editar`}>
                    <Button size="sm" variant="outline" className="gap-1"><Edit2 size={12} /> Editar</Button>
                  </Link>
        ...
        <div className="flex bg-slate-100 rounded-xl p-1">
          {[['diary','📖 Diário'],['weight','⚖️ Peso']] .map(([t, label]) => (
            <button key={t} type="button"
              onClick={() => setTab(t as 'diary'|'weight')}
        ...
        {tab === 'diary' && (
          <div>
            ...
            {entries.length === 0 ? (
              <Card><CardContent className="p-8 text-center text-slate-400 text-sm">Nenhum registro no diário ainda</CardContent></Card>
            ) : (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button onClick={() => deleteEntry(e.id)} className="text-slate-300 hover:text-red-500 flex-shrink-0">
                              <Trash2 size={14} />
                            </button>
                          </div>
      ...
      <Modal open={modalDiary} onClose={() => setModalDiary(false)} title="Novo registro">
        <div className="space-y-3">
          ...
          {['vaccine','deworming','medication'].includes(diaryForm.type) && (
            <Input label="Produto / medicamento" placeholder="Ex: Multivac" value={diaryForm.product_name}
              onChange={e => setDiaryForm(p => ({ ...p, product_name: e.target.value }))} />
          )}
          {['appointment','exam','surgery'].includes(diaryForm.type) && (
            <Input label="Veterinário" placeholder="Nome do vet" value={diaryForm.vet_name}
          ...
          <div>
            ...
            <textarea className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
              rows={2} placeholder="Detalhes do registro..." value={diaryForm.description}
              onChange={e => setDiaryForm(p => ({ ...p, description: e.target.value }))} />
          </div>
      ...
      <Modal open={modalWeight} onClose={() => setModalWeight(false)} title="Registrar peso">
        <div className="space-y-3">
          <Input label="Peso (kg) *" type="number" step="0.1" placeholder="Ex: 8.5" value={weightForm.weight_kg}
          ...
          <Input label="Observações" placeholder="Ex: Pós-banho" value={weightForm.notes}
      ...
      <Modal open={modalDelete} onClose={() => setModalDelete(false)} title="Excluir pet">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mb-5">Esta ação não pode ser desfeita. Todos os registros do pet serão perdidos.</p>
