...
export function Sidebar({ onLogout, planLabel = 'Grátis' }: SidebarProps) {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 min-h-screen bg-white border-r border-slate-100 fixed top-0 left-0 z-40">
      {/* Logo */}
      <div className="px-6 py-5 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-slate-800 text-base leading-none">MeuPet<span className="text-blue-600">+</span></div>
            <div className="text-[10px] text-slate-400 mt-0.5 font-medium uppercase tracking-wide">{planLabel}</div>
          </div>
        </div>
      </div>
